...
test_watcher_cleans_up_state() {
  ...
  # State files should be cleaned up by the EXIT trap.
  # The empty directory itself may survive on some CI runners (macOS GitHub Actions)
  # where background subshell EXIT traps race with wait, so we check files not dir.
  local leftover_files
  leftover_files="$(find "$state_dir" -type f 2>/dev/null | head -5)"
  if [[ -n "$leftover_files" ]]; then
    echo -e "  ${RED}FAIL: state files should be removed after watcher exits${NC}" >&2
    echo -e "    leftover: $leftover_files" >&2
    rm -rf "$state_dir"
    rm -f "$transcript"
    return 1
  fi
